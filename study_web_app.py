...
# 1. 페이지 설정
st.set_page_config(page_title="감평 인출 훈련기", layout="wide")
...
<style>
    ...
    .question-text { font-size: 4.0rem !important; font-weight: bold; color: #f1c40f; text-align: center; margin: 30px 0; line-height: 1.3; }
    .answer-text { font-size: 4.0rem !important; font-weight: bold; color: #2ecc71; text-align: center; margin: 30px 0; line-height: 1.3; }
...
if df is not None:
    ...
    with col:
        if st.session_state.current_index == "GRADUATED":
            ...
            st.write(f"현재 누적 정답: {total_correct}/5")
            ...
            with c1:
                if st.button("맞음 (O)", type="primary"):
                    ...
                    if is_pc: # PC에서 시트 업데이트 시도 (에러 방어막)
                        try:
