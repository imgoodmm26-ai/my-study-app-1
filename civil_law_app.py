...
# 3. 디자인 설정 (PC 2/3, 모바일 1/2 축소 완벽 반영)
st.markdown("""
<style>
    ...
    .gauge-row { font-size: 1.8rem; font-family: 'Courier New', monospace; display: flex; align-items: center; justify-content: center; white-space: nowrap; width: 100%; }
...
with st.sidebar:
    st.markdown("### 📂 학습 범위 선택")
    if sheet_list:
        if st.session_state.sheet_name is None: st.session_state.sheet_name = sheet_list[0]
        selected = st.selectbox("시트를 선택하세요", sheet_list, index=sheet_list.index(st.session_state.sheet_name) if st.session_state.sheet_name in sheet_list else 0)
    else:
        st.warning("자동 시트 목록을 가져오지 못했습니다. (공유 설정 확인 요망)")
        selected = st.text_input("공부할 시트 이름을 직접 입력하세요:", value="시트18")
...
if df is not None:
    ...
    with col:
        ...
        if st.session_state.current_index == "GRADUATED":
            ...
            st.markdown(f'<p class="question-text">[{st.session_state.sheet_name}] 훈련 시스템</p>', unsafe_allow_html=True)
