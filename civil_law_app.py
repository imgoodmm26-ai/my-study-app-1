...
# 1. 페이지 설정
st.set_page_config(page_title="감평 와이드 인출기: 커스텀 에디션", layout="wide")
...
if 'last_msg' not in st.session_state: st.session_state.last_msg = "버튼 레이아웃 최적화 완료. 훈련을 계속하세요!"

# 3. 디자인 설정 (동기화 버튼 축소 및 12cm/3cm 여백 분리 적용)
st.markdown("""
<style>
    ...
    /* 하단 그래프 여백 설정 (버튼으로부터 딱 3cm ≈ 120px) */
    .progress-container { 
        width: 100%; 
        background-color: #222; 
        border-radius: 10px; 
        margin-top: 120px; /* 버튼과 그래프 간격 3cm */
        display: flex; 
        height: 12px; 
        overflow: hidden; 
    }
...
if df is not None:
    ...
    with t_col2:
        if st.button("🔄 데이터 동기화", key="sync_btn"):
            st.cache_data.clear()
            st.session_state.df = load_data()
            st.session_state.last_msg = f"동기화 완료: {len(st.session_state.df)}문항 수신됨"
            st.rerun()
    ...
    with col:
        ...
        if st.session_state.current_index == "GRADUATED":
            ...
            label = f'<div style="text-align:center;"><span class="status-badge badge-new">🆕 신규 문항</span></div>' if c_lv == 0 else f'<div style="text-align:center;"><span class="status-badge badge-review">🔥 Lv.{c_lv} 복습 중</span></div>'
